...
test('creates middleware factories for Nest request methods', async () => {
	...
	const headFactory = await adapter.createMiddlewareFactory(RequestMethod.HEAD)
	...
	expect((await adapter.hono.request('/head', { method: 'HEAD' })).status).toBe(
		200
	)
	await expect((await adapter.hono.request('/fallback')).text()).resolves.toBe(
		'fallback'
	)
})
